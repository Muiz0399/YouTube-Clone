  ...
  return (
    <>
      <div
        className={`${
          mobileShow
            ? "mt-12 fixed top-0 bottom-0 left-0 transition-all duration-300 bg-white z-40 h-screen w-[50%] sm:w-[38%]"
            : "mt-16 hidden h-[calc(100vh)] w-[32%]"
            ...
            return (
              <div className="mb-5">
                ...
                {group.groupItems.map((item) => (
                  ...
                  >
                    <div className="text-md cursor-pointer">{item.icon}</div>
                    <span className="text-md cursor-pointer">{item.name}</span>
                  </div>
                ))}
              </div>
            );
